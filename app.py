...
║         Model loaded from: xgboost.pkl (pre-trained)                ║
╚══════════════════════════════════════════════════════════════════════╝

HOW TO RUN:
  1. Place this file in the SAME FOLDER as:
       xgboost.pkl
  2. Run:  streamlit run app_xgboost_only.py
"""
...
.hero-title  { font-size: 2.8rem; font-weight: 900; color: #ffffff; margin: 0; text-shadow: 0 4px 20px rgba(0,0,0,0.3); letter-spacing: -1px; }
...
.stSelectbox label, .stSlider label, .stNumberInput label { color: #d1d5db !important; font-weight: 600; }
.success-alert { background: rgba(16,185,129,0.15); border: 1px solid rgba(16,185,129,0.4); border-radius: 10px; padding: 1rem 1.2rem; color: #6ee7b7; }
.warning-alert { background: rgba(245,158,11,0.15); border: 1px solid rgba(245,158,11,0.4); border-radius: 10px; padding: 1rem 1.2rem; color: #fcd34d; }
.pkl-status-ok  { background: rgba(16,185,129,0.12); border:1px solid rgba(16,185,129,0.35); border-radius:10px; padding:0.7rem 1rem; color:#6ee7b7; margin-bottom:0.4rem; font-size:0.9rem; }
.pkl-status-err { background: rgba(239,68,68,0.12); border:1px solid rgba(239,68,68,0.35); border-radius:10px; padding:0.7rem 1rem; color:#fca5a5; margin-bottom:0.4rem; font-size:0.9rem; }
</style>
...
# ─────────────────────────────────────────────────────────────────────
# PKL LOADER  — XGBoost ONLY
# ─────────────────────────────────────────────────────────────────────
XGBOOST_FILE = "xgboost.pkl"

@st.cache_resource(show_spinner=False)
def load_xgboost_model():
    """Load pre-trained XGBoost model from xgboost.pkl."""
...
# SESSION STATE
# ─────────────────────────────────────────────────────────────────────
for key in ['df', 'eval_results', 'X_test', 'y_test', 'fitted_scaler']:
...
# LOAD MODEL (cached)
# ─────────────────────────────────────────────────────────────────────
xgb_model, xgb_ok, xgb_err = load_xgboost_model()
model_loaded = xgb_ok

# Standard feature columns (creditcard.csv order)
...
with st.sidebar:
    ...
    # Show pkl load status in sidebar
    st.markdown("<div style='font-size:0.78rem;color:#a78bfa;font-weight:700;margin-bottom:0.4rem'>📦 PKL File Status</div>", unsafe_allow_html=True)
    if xgb_ok:
        st.markdown("<div style='font-size:0.75rem;color:#6ee7b7'>✅ XGBoost</div>", unsafe_allow_html=True)
    else:
        st.markdown(f"<div style='font-size:0.75rem;color:#fca5a5'>❌ XGBoost — {xgb_err}</div>", unsafe_allow_html=True)
    ...
    <div style="font-size:0.75rem; color:#6b7280; text-align:center">
      ...
      <b style="color:#a78bfa">Model:</b> XGBoost (xgboost.pkl)<br><br>
      <b style="color:#a78bfa">Techniques:</b> StandardScaler (auto-fit)
    </div>
    """, unsafe_allow_html=True)
...
if page == "🏠 Home & Dataset":
    ...
    <div class="section-header">
      ...
      <div><h2>Load Dataset</h2><p>Upload creditcard.csv from Kaggle (optional – needed for EDA & Evaluation)</p></div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if uploaded:
        ...
        st.markdown('<div class="success-alert">✅ Dataset loaded and duplicates removed!</div>', unsafe_allow_html=True)
    ...
    <div class="section-header">
      ...
      <div><h2>Pre-trained Model Status</h2><p>XGBoost model loaded automatically from xgboost.pkl</p></div>
    </div>
    """, unsafe_allow_html=True)

    if xgb_ok:
        st.markdown('<div class="pkl-status-ok">✅ <b>XGBoost</b> — xgboost.pkl loaded successfully</div>', unsafe_allow_html=True)
    else:
        st.markdown(f'<div class="pkl-status-err">❌ <b>XGBoost</b> — {xgb_err} &nbsp;(place <code>xgboost.pkl</code> in same folder as this script)</div>', unsafe_allow_html=True)
        st.markdown("""
        <div class="warning-alert">
          ⚠️ <b>xgboost.pkl</b> not found!<br>
          Make sure <code>xgboost.pkl</code> is in the <b>same folder</b> as this script.
        </div>
        """, unsafe_allow_html=True)
    ...
    if df is not None:
        ...
        <div class="warning-alert" style="margin-top:1rem">
          ⚠️ Upload <b>creditcard.csv</b> for EDA & evaluation features.<br>
          The Predict page works without the dataset — just the XGBoost model.<br>
          Dataset: <a href="https://www.kaggle.com/datasets/mlg-ulb/creditcardfraud/data"
          target="_blank" style="color:#fbbf24">Kaggle ULB Credit Card Fraud</a>
        </div>
        """, unsafe_allow_html=True)
...
# ═════════════════════════════════════════════════════════════════════
    ...
    df = st.session_state.df
    if df is None:
        st.warning("⚠️ Please upload the dataset on the Home page first.")
        st.stop()
    ...
    with tab1:
        ...
        fig.add_trace(go.Pie(
            labels=['Non-Fraud', 'Fraud'], values=counts.values,
            hole=0.55, pull=[0, 0.12],
            marker=dict(colors=['#667eea', '#f64f59'], line=dict(color='rgba(0,0,0,0)', width=0)),
        ), row=1, col=1)
        fig.add_trace(go.Bar(
            x=['Non-Fraud', 'Fraud'], y=counts.values,
            marker=dict(color=['#667eea', '#f64f59'], line=dict(width=0)),
            text=[f"{v:,}" for v in counts.values], textposition='auto'
        ), row=1, col=2)
        fig.update_layout(title="Fraud vs Legitimate Transactions", **DARK,
                          height=420, showlegend=False, title_font_size=16)
        st.plotly_chart(fig, use_container_width=True)

        col1, col2 = st.columns(2)
        with col1:
            st.markdown(f'<div class="info-card" style="border-left-color:#10b981"><h4>✅ Legitimate</h4>'
                        f'<p><b style="font-size:1.4rem;color:#10b981">{counts[0]:,}</b>&nbsp;({counts[0]/len(df)*100:.2f}%)</p></div>',
                        unsafe_allow_html=True)
        with col2:
            st.markdown(f'<div class="info-card" style="border-left-color:#ef4444"><h4>🚨 Fraudulent</h4>'
                        f'<p><b style="font-size:1.4rem;color:#ef4444">{counts[1]:,}</b>&nbsp;({counts[1]/len(df)*100:.4f}%)</p></div>',
                        unsafe_allow_html=True)

    with tab2:
        fig1 = px.histogram(df, x='Time', color='Class', nbins=60, barmode='overlay',
                            title='Transaction Time Distribution by Class',
                            labels={'Class': 'Class', 'Time': 'Time (seconds)'},
                            color_discrete_map={0: '#667eea', 1: '#f64f59'}, opacity=0.75)
        fig1.update_layout(**DARK, height=380, title_font_size=16)
        st.plotly_chart(fig1, use_container_width=True)

        data_grouped = df.groupby('Time')['Amount'].mean().reset_index()
        fig2 = px.line(data_grouped, x='Time', y='Amount',
                       title='Average Transaction Amount Over Time',
                       color_discrete_sequence=['#a78bfa'])
        fig2.update_layout(**DARK, height=350, title_font_size=16)
    ...
    with tab3:
        ...
        fig.update_layout(**DARK, height=420, title_font_size=16, showlegend=False)
        st.plotly_chart(fig, use_container_width=True)

        fig = px.violin(df.sample(min(5000, len(df))), x='Class', y='V1',
                        color='Class', box=True,
                        title='Distribution of V1 by Class (sample)',
                        color_discrete_map={0: '#667eea', 1: '#f64f59'})
        fig.update_layout(**DARK, height=420, showlegend=False, title_font_size=16)
        st.plotly_chart(fig, use_container_width=True)

    with tab4:
        corr_cols = [c for c in df.columns if c not in ['Time']]
        sample_df = df[corr_cols].sample(min(10000, len(df)))
    ...
    with tab5:
        numerical_df   = df.select_dtypes(include=['number']).drop(columns=['Class'])
        Q1, Q3         = numerical_df.quantile(0.25), numerical_df.quantile(0.75)
        IQR            = Q3 - Q1
        outliers_count = ((numerical_df < (Q1 - 1.5*IQR)) | (numerical_df > (Q3 + 1.5*IQR))).sum()
        fig = px.bar(x=outliers_count.index, y=outliers_count.values,
                     title='Outlier Count per Feature (IQR Method)',
                     labels={'x': 'Feature', 'y': 'Outlier Count'},
                     color=outliers_count.values, color_continuous_scale='RdPu')
        fig.update_layout(**DARK, height=420, title_font_size=16)
        st.plotly_chart(fig, use_container_width=True)
        st.caption(f"Total outliers detected: **{outliers_count.sum():,}** across all features")
...
# ═════════════════════════════════════════════════════════════════════
    ...
    <div class="section-header">
      ...
      <p>Model loaded from xgboost.pkl — no training needed!</p></div>
    </div>
    """, unsafe_allow_html=True)

    # PKL load report
    st.markdown("#### 📦 PKL File Load Report")
    if xgb_ok:
        st.markdown('<div class="pkl-status-ok">✅ <b>XGBoost</b> loaded from <code>xgboost.pkl</code></div>', unsafe_allow_html=True)
    else:
        st.markdown(f'<div class="pkl-status-err">❌ <b>XGBoost</b> — {xgb_err} &nbsp;(<code>xgboost.pkl</code> must be in same folder as this script)</div>', unsafe_allow_html=True)
    ...
    <div class="section-header">
      ...
      <div><h2>Evaluate on Dataset</h2>
      <p>Upload creditcard.csv to benchmark XGBoost on real test data</p></div>
    </div>
    """, unsafe_allow_html=True)

    df = st.session_state.df
    if df is None:
        st.markdown('<div class="warning-alert">⚠️ Upload <b>creditcard.csv</b> on the Home page to run evaluation.</div>', unsafe_allow_html=True)
        st.stop()
    ...
    if st.button("🔍 Evaluate XGBoost Model"):
        ...
        with st.spinner("⏳ Running evaluation…"):
            ...
            # Fit a StandardScaler on test split (no scaler.pkl needed)
            sc = _SS()
    ...
    if st.session_state.eval_results:
        ...
        display_keys = ["Model", "Accuracy", "Precision", "Recall", "F1 Score", "ROC AUC"]
        results_df = pd.DataFrame([{k: r[k] for k in display_keys}])
        ...
        for metric, color in zip(metrics, colors):
            fig.add_trace(go.Bar(
                name=metric, x=["XGBoost"], y=[r[metric]],
                marker_color=color,
                text=[f"{r[metric]:.4f}"], textposition='auto'
            ))
        ...
        <div style="background:linear-gradient(135deg,rgba(102,126,234,0.2),rgba(118,75,162,0.2));
          ...
          <div style="font-size:2rem;font-weight:900;color:#fff;margin-top:0.3rem">XGBoost</div>
...
# ═════════════════════════════════════════════════════════════════════
    ...
    if not model_loaded:
        st.warning("⚠️ xgboost.pkl not found. Place it in the same folder as this script.")
        st.stop()
    ...
    <div class="section-header">
      ...
      <p>Enter transaction details to get instant fraud probability from XGBoost</p></div>
    </div>
    """, unsafe_allow_html=True)
    ...
    col1, col2 = st.columns(2)
    with col1:
        time_val   = st.number_input("⏰ Time (seconds)", value=0.0, format="%.2f")
    with col2:
        amount_val = st.number_input("💵 Amount ($)", value=100.0, min_value=0.0, format="%.2f")
    ...
    if st.button("🔍 Analyse Transaction"):
        ...
        # Use fitted scaler from evaluation if available, else fit on input
        if st.session_state.fitted_scaler is not None:
            try:
                input_sc = st.session_state.fitted_scaler.transform(input_df)
            except Exception:
                input_sc = input_df.values
        else:
            # No scaler available — use raw values with a note
            sc_temp  = _SS()
            input_sc = sc_temp.fit_transform(input_df)
            st.markdown('<div class="warning-alert">⚠️ No fitted scaler found. Run Evaluation first for best accuracy. Using auto-scaled values.</div>', unsafe_allow_html=True)
        ...
        if prediction == 1:
            ...
            <div class="result-fraud">
              ...
              <span style="font-size:1rem;font-weight:400">Fraud Probability: {fraud_prob:.1f}%</span>
            </div>""", unsafe_allow_html=True)
        else:
            st.markdown(f"""
            <div class="result-legit">
              ✅ LEGITIMATE TRANSACTION<br>
              <span style="font-size:1rem;font-weight:400">Legitimate Probability: {legit_prob:.1f}%</span>
            </div>""", unsafe_allow_html=True)
        ...
        fig = go.Figure(go.Indicator(
            ...
            value=fraud_prob,
            title={'text': "Fraud Probability (%)", 'font': {'color': 'white', 'size': 16}},
            delta={'reference': 50},
            gauge={
        ...
        with c1:
            st.markdown('<div class="metric-card"><div class="metric-icon">⚡</div><div class="metric-value" style="font-size:1rem">XGBoost</div><div class="metric-label">Model Used</div></div>', unsafe_allow_html=True)
        with c2:
            st.markdown(f'<div class="metric-card"><div class="metric-icon">✅</div><div class="metric-value">{legit_prob:.1f}%</div><div class="metric-label">Legit Probability</div></div>', unsafe_allow_html=True)
        with c3:
            st.markdown(f'<div class="metric-card"><div class="metric-icon">🚨</div><div class="metric-value">{fraud_prob:.1f}%</div><div class="metric-label">Fraud Probability</div></div>', unsafe_allow_html=True)
...
# ═════════════════════════════════════════════════════════════════════
    ...
    if not results:
        st.warning("⚠️ No evaluation results yet. Go to **Model Status & Evaluate**, upload the dataset, and run evaluation first.")
        st.stop()
    ...
    with tab3:
        if hasattr(xgb_model, 'feature_importances_'):
            ...
            fig.update_layout(**DARK, height=560, title_font_size=16)
            fig.update_yaxes(autorange='reversed')
            st.plotly_chart(fig, use_container_width=True)
        else:
            st.info("Feature importances not available for this model.")

        # Metrics radar (single model)
        st.markdown("#### 🕸️ XGBoost Metrics Radar")
        metrics_list = ["Accuracy", "Precision", "Recall", "F1 Score", "ROC AUC"]
        values = [r[m] for m in metrics_list] + [r[metrics_list[0]]]
        fig = go.Figure()
        fig.add_trace(go.Scatterpolar(
            r=values, theta=metrics_list + [metrics_list[0]],
            fill='toself', name='XGBoost',
            line_color='#667eea', fillcolor='rgba(102,126,234,0.2)'
        ))
        fig.update_layout(
            polar=dict(
...
<div style="text-align:center; padding: 1.5rem; color:#6b7280; font-size:0.82rem">
  ...
  Dataset: <a href="https://www.kaggle.com/datasets/mlg-ulb/creditcardfraud/data"
  target="_blank" style="color:#667eea">Kaggle ULB Credit Card Fraud Detection</a><br>
  <span style="color:#a78bfa">⚡ Powered by XGBoost (xgboost.pkl) — no retraining needed</span>
</div>
""", unsafe_allow_html=True)